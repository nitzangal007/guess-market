...
`XmlMarketLoader` is the public XML file boundary. Its only public operation, `load(Path)`, returns a fresh mapper-produced `LinkedHashMap<Integer, MarketEvent>` or throws the checked `EngineOperationException`. It never reaches live Engine state. The method first rejects a null path, a filename without a case-insensitive `.xml` suffix, and a nonregular existing path as `INVALID_XML_PATH`. A missing matching file becomes `XML_FILE_NOT_FOUND`; a known unreadable file, stream-open failure, or I/O failure while reading becomes `XML_FILE_ACCESS_FAILED`.
...
After successful structure validation, `load` invokes the package-private mapper boundary. Mapper-owned business failures remain unchanged as `XML_DATA_INVALID`, including their event number, actual event ID, and field context. The loader catches only checked expected I/O and JAXB/schema failures. It does not catch `RuntimeException` or `Exception`, so programming defects stay visible. Two package-private functional seams exist only for deterministic same-package tests: the default production path remains `Files::newInputStream` and `JaxbMarketMapper::map`, while the tests can make the real loader path receive an expected `IOException` or a deliberate mapper defect without reflection or a public test API.

`XmlMarketLoaderTest` has 13 active tests. It covers null, directory, bad suffix, uppercase `.XML`, a path containing spaces, missing files, a misleading XML schema-location hint, malformed XML, missing schema structure, mapper-owned business context, deterministic expected stream-open I/O translation, visible mapper defects, and input closure after both success and failure. The closure tests move and delete the input after `load` returns or throws, which is observable on Windows. Windows does not provide a stable portable way for these tests to make a normal temporary file unreadable to its owner, so no skipped or disabled permission test was added. Instead, the deterministic stream-open `IOException` executes the public `load(Path)` path through the same try-with-resources and error translation code that a genuine access failure uses.

The focused gate compiled all DTO and Engine source and test files with Java 25, `--release 25 -encoding UTF-8 -Xlint:all -Werror`, JUnit Platform Console Standalone 6.1.1, and exactly the five approved JAXB runtime JARs. It then selected `guessmarket.engine.xml.XmlMarketLoaderTest`: 13 tests found, started, and successful, with zero failures, skips, or aborts. The six-suite regression repeated that strict compile and selected DTO, Engine contract, LMSR, domain, mapper, and loader suites. It reported 70 tests found, started, and successful, with zero failures, skips, or aborts.
...
| 3 | `XmlMarketLoader` | `validatePath` and `hasXmlSuffix` | Classify null, case-insensitive suffix, existence, regular-file, and readability conditions into the three approved path and access codes before opening the input | Loader null, directory, suffix, uppercase, missing, and deterministic I/O tests | Implemented |
| 3 | `XmlMarketLoader` | `trustedSchema` and `trustedUnmarshaller` | Load the Engine-controlled classpath XSD, configure JAXB validation, and report only installation or JAXB setup defects as `ENGINE_CONFIGURATION_ERROR` | Trusted-hint, valid-load, malformed, and missing-structure loader tests | Implemented |
...
| 3 | Task 6 JAXB resources and generated source, Task 7 business mapping, and Task 8 trusted-schema loading | `JaxbMarketMapperTest`: 9 successful; `XmlMarketLoaderTest`: 13 successful | Fresh Java 25 strict compile with `-Xlint:all -Werror`; all six current suites reported 70 successful tests with zero failures, skips, disabled tests, or aborts | Task 6 `43c67d4`; Task 7 `32c6a76`; Task 8 `feat: load markets with trusted XML schema` | Ready for review |
