...
`ConsoleInput` separates prompt-local text handling from the later command loop. Each reader consumes one complete `Scanner` line, trims only outer whitespace, and repeats only its own prompt after a blank, non-numeric, overflowing, zero, negative, or out-of-range integer. The method names distinguish the one-shot menu choice, current displayed event position, purchase option, winning option, positive quantity, and three full-path conversations. The XML, save, and restore readers return `Path` values while preserving spaces inside a supplied filename. A nested checked `EndOfInputException` makes normal input closure different from invalid user text without adding another production UI class. Neither injected stream is closed.

`ConsoleRenderer` centralizes all deterministic presentation. Two named 60-character ASCII constants prevent the menu's equals separator and top-level hyphen separator from drifting. Summary blocks preserve the DTO's load order and XML text while assigning fresh one-based display positions. Detail rendering keeps `OPTIONS`, `ACCOUNT`, and `PURCHASE HISTORY` as subsections, shows the winner only for a closed snapshot, and derives profit, subsidy, or break-even from the unrounded account-balance sign. `formatFinancial` is the one presentation-only money formatter: it uses `Locale.US`, preserves two decimal places, normalizes rendered negative zero, and rejects non-finite values as programming defects.

The Task 11 RED compile used the two new test classes before either production type existed. It failed with 23 expected missing-symbol errors for `ConsoleInput` and `ConsoleRenderer`. The final focused gate compiled the DTOs, required Engine exception types, Task 11 UI source, and the two test classes with Oracle Java 25.0.4 using `--release 25 -encoding UTF-8 -Xlint:all -Werror`. `ConsoleInputTest` and `ConsoleRendererTest` then reported 13 successful tests with zero failures, skips, or aborts. The tests cover exact revised prompts and parsing messages, retry scope, path spaces, end-of-input, the complete menu literal, Clean Sections headings and separators, full and filtered summaries, long text preservation, both commission modes, closed-result categories, newest-first history, every Engine error code, omission of raw exception text, ASCII-only output, and financial formatting.
...
| 5 | `ConsoleInput` | Menu, selection, option, quantity, and path readers | Read full lines, print the exact D-072 range prompts, repeat prompt-local shape errors, preserve internal path spaces, and signal end-of-input without closing borrowed streams | `ConsoleInputTest` | Implemented |
...
| 5 | `ConsoleRenderer` | Menu, DTO, receipt, history, and error renderers | Produce the deterministic D-072 Clean Sections menu, named ASCII blocks, financial categories, and safe error mappings without domain logic | `ConsoleRendererTest` | Implemented |
...
| 5 | Task 11 input and rendering foundation | `ConsoleInputTest`: 6 successful; `ConsoleRendererTest`: 7 successful | Oracle Java 25.0.4 strict focused compile and JUnit gate: 13 successful with zero failures, skips, or aborts | Pending Task 11 commit | Implemented, awaiting Task 12 |
