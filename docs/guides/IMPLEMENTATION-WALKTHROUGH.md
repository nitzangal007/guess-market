...
`GuessMarketEnginePersistenceTest` is a same-package boundary test because Task 10 has not yet created the public Engine implementation. Its ten cases cover suffix and path behavior, first save, overwrite, injected atomic and fallback publication paths, failed publication cleanup, missing and corrupt files, wrong and filtered roots, format version, empty and aliased candidates, duplicate IDs, history-quantity and commission-formula inconsistencies reached through serialized state, both `int` ID endpoints, and a tiny representable stored history cost. The RED command compiled the new test against the pre-Task-9 production source and failed with 23 expected missing-symbol errors for `SavedState`, `SavedStateValidator`, and `SavedStateStore`. A self-review then added a commission-formula RED test, which correctly failed before the narrow validation correction. The focused strict Java 25 gate then reported 10 successful tests. The seven-suite regression gate reported 84 successful tests, zero failures, skips, or aborts, with exactly the five approved JAXB runtime JARs.
...
| 4 | Task 9 private saved-state root, strict filter, validation quarantine, and publication behavior. Task 10 Engine orchestration remains pending. | `GuessMarketEnginePersistenceTest`: 10 successful | Fresh Java 25 strict compile with `-Xlint:all -Werror`; seven current suites used exactly five JAXB runtime JARs; 84 tests successful with zero failures, skips, disabled tests, or aborts | Task 9 commit pending | In progress |
