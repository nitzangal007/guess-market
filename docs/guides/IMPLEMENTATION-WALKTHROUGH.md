...
`XmlMarketLoader` is the public XML file boundary. Its only public operation, `load(Path)`, returns a fresh mapper-produced `LinkedHashMap<Integer, MarketEvent>` or throws the checked `EngineOperationException`. It never reaches live Engine state. The method first rejects a null path and a filename without a case-insensitive `.xml` suffix. It then reads `BasicFileAttributes` once: `NoSuchFileException` becomes `XML_FILE_NOT_FOUND`, any other checked metadata-read I/O failure becomes `XML_FILE_ACCESS_FAILED`, and a confirmed nonregular path becomes `INVALID_XML_PATH`. A known unreadable file, stream-open failure, or I/O failure while reading also becomes `XML_FILE_ACCESS_FAILED`.
...
`XmlMarketLoaderTest` has 16 active tests. It covers null, a directory explicitly named `folder.xml`, bad suffix, uppercase `.XML`, a path containing spaces, missing files, a misleading XML schema-location hint, malformed XML, missing schema structure, missing, unreadable, and malformed trusted schemas, mapper-owned business context, deterministic expected stream-open I/O translation, visible mapper defects, and input closure after both success and failure. The closure tests move and delete the input after `load` returns or throws, which is observable on Windows. Windows does not provide a stable portable way for these tests to make a normal temporary file unreadable to its owner, so no skipped or disabled permission test was added. Instead, the deterministic stream-open `IOException` executes the public `load(Path)` path through the same try-with-resources and error translation code that a genuine access failure uses.

The focused gate compiled all DTO and Engine source and test files with Java 25, `--release 25 -encoding UTF-8 -Xlint:all -Werror`, JUnit Platform Console Standalone 6.1.1, and exactly the five approved JAXB runtime JARs. It then selected `guessmarket.engine.xml.XmlMarketLoaderTest`: 16 tests found, started, and successful, with zero failures, skips, or aborts. The six-suite regression repeated that strict compile and selected DTO, Engine contract, LMSR, domain, mapper, and loader suites. It reported 73 tests found, started, and successful, with zero failures, skips, or aborts.
...
| 3 | `XmlMarketLoader` | `load` | Validate the path, create a fresh trusted-schema JAXB unmarshaller, close the XML stream, map the complete candidate, and preserve mapper failures without touching live state | All 16 `XmlMarketLoaderTest` cases | Implemented |
...
| 3 | Task 6 JAXB resources and generated source, Task 7 business mapping, and Task 8 trusted-schema loading | `JaxbMarketMapperTest`: 9 successful; `XmlMarketLoaderTest`: 16 successful | Fresh Java 25 strict compile with `-Xlint:all -Werror`; all six current suites reported 73 successful tests with zero failures, skips, disabled tests, or aborts | Task 6 `43c67d4`; Task 7 `32c6a76`; Task 8 `feat: load markets with trusted XML schema` plus review fix | Ready for review |
